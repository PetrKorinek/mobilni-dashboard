...
for ticker in tickery:
    try:
        ...
        if data.empty:
            st.warning(f"{ticker}: prázdná data.")
            continue

        # Extrakce správného sloupce 'Close'
        if isinstance(data.columns, pd.MultiIndex):
            if (ticker, "Close") in data.columns:
                close_series = data[(ticker, "Close")]
            else:
                st.warning(f"{ticker}: sloupec ('{ticker}', 'Close') není ve stažených datech.")
                continue
        else:
            if "Close" in data.columns:
                close_series = data["Close"]
            else:
                st.warning(f"{ticker}: sloupec 'Close' není ve stažených datech.")
                continue

        if close_series.empty or len(close_series) < 50:
            st.warning(f"{ticker}: nedostatečná data.")
            continue
